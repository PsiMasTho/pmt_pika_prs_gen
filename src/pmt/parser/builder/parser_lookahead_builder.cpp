  ...
  switch (kind_) {
   case SymbolKindTerminal:
    symbols_.for_each_key([&](SymbolValueType symbol_) {
     ret += std::exchange(delim, ", ") + args_._fn_lookup_terminal_label(symbol_);
    });
    break;
   default:
    symbols_.for_each_key([&](SymbolValueType symbol_) {
...
void try_solve_partial_conflict(ParserLookaheadBuilder::Args const& args_, Locals& locals_, StateMachine& state_machine_, IntervalSet<SymbolValueType> conflict_symbols_) {
 ...
 while (!pending.empty()) {
   ...
   switch (state_cur.get_accepts().popcnt()) {
    ...
    case 0:
     state_cur.get_symbol_transitions(SymbolKindTerminal).for_each_interval([&](StateNrType const state_nr_next_, Interval<SymbolValueType> const) {
 ...
 if (!conflict_symbols_.empty()) {
  ...
  std::string delim;
  conflict_symbols_.for_each_key([&](SymbolValueType symbol_) {
   msg += std::exchange(delim, ", ") + "conflict_" + std::to_string(symbol_);
  });
...
void setup_lookahead_state_machine(ParserLookaheadBuilder::Args const& args_, Locals& locals_) {
 ...
 args_._parser_state_machine.get_state_nrs().for_each_key([&](StateNrType state_nr_) {
  ...
  close_symbols.for_each_key([&](StateNrType const state_nr_next_, SymbolValueType const symbol_) {
   post_close_state_nrs[symbol_].insert(state_nr_next_);
  });
 });
  ...
  state_conflicting.get_symbol_transitions(SymbolKindConflict).for_each_key([&](StateNrType const state_nr_post_conflcit_, SymbolValueType const conflict_symbol_) {
   ...
   while (!pending.empty()) {
    ...
    // SymbolKindTerminal
     state_old.get_symbol_transitions(SymbolKindTerminal).for_each_interval([&](StateNrType const state_nr_next_old_, Interval<SymbolValueType> const interval_) {
      state_new.add_symbol_transition(SymbolKindTerminal, interval_, push_and_visit(state_nr_next_old_));
     });

    // SymbolKindHiddenTerminal
    state_old.get_symbol_transitions(SymbolKindHiddenTerminal).for_each_interval([&](StateNrType const state_nr_next_old_, Interval<SymbolValueType> const interval_) {
     state_new.add_symbol_transition(SymbolKindTerminal, interval_, push_and_visit(state_nr_next_old_));
    });

    // SymbolKindOpen
    state_old.get_symbol_transitions(SymbolKindOpen).for_each_interval([&](StateNrType const state_nr_next_old_, Interval<SymbolValueType> const interval_) {
     state_new.add_epsilon_transition(push_and_visit(state_nr_next_old_));
    });
  ...
  partial_conflict_state_machine.get_state_nrs().for_each_key([&](StateNrType state_nr_) {
   locals_._lookahead_state_machine.get_or_create_state(state_nr_) = *partial_conflict_state_machine.get_state(state_nr_);
  });
 });
}
...
auto ParserLookaheadBuilder::extract_conflicts(StateMachine& state_machine_) -> IntervalSet<StateNrType> {
 ...
 ret.for_each_key([&](StateNrType state_nr_) {
  ...
  if (!terminal_symbols.empty()) {
   ...
   state_nrs_new.insert(Interval<StateNrType>(state_nr_new));
   State& state_new = *state_machine_.get_state(state_nr_new);
   terminal_symbols.for_each_interval([&](StateNrType const state_nr_next_, Interval<SymbolValueType> const interval_) {
    state_new.add_symbol_transition(SymbolKindTerminal, interval_, state_nr_next_);
   });
  }
  ...
  if (!hidden_terminal_symbols.empty()) {
   ...
   hidden_terminal_symbols.for_each_interval([&](StateNrType const state_nr_next_, Interval<SymbolValueType> const interval_) {
    state_new_hidden.add_symbol_transition(SymbolKindHiddenTerminal, interval_, state_nr_next_);
   });
  }
  ...
  if (!close_symbols.empty()) { 
   ...
   close_symbols.for_each_interval([&](StateNrType const state_nr_next_, Interval<SymbolValueType> const interval_) {
    state_new_open_close.add_symbol_transition(SymbolKindClose, interval_, state_nr_next_);
   });

   open_symbols.for_each_interval([&](StateNrType const state_nr_next_, Interval<SymbolValueType> const interval_) {
    state_new_open_close.add_symbol_transition(SymbolKindOpen, interval_, state_nr_next_);
   });
  }

 ...
 interval_.for_each_key([&](SymbolValueType symbol_) {
  ...
  // Search for the accepting state and point it to the next state when found
  {
   ...
   while (!pending.empty()) {
    ...
    state_cur.get_symbol_transitions(SymbolKindCharacter).for_each_interval([&](StateNrType next_state_nr_, Interval<SymbolValueType> interval_) {
     pending.push_back(next_state_nr_);
    });
   }
  }
...
void follow_and_copy_symbol_transitions(TerminalOverlapChecker::Args& args_, Locals& locals_, State const& state_old_, State& state_new_) {
 state_old_.get_symbol_transitions(SymbolKindTerminal).for_each_interval([&](StateNrType state_nr_next_old_, Interval<SymbolValueType> interval_) {
  follow_and_copy_terminal_transitions(args_, locals_, interval_, state_nr_next_old_, state_new_);
 });

 state_old_.get_symbol_transitions(SymbolKindHiddenTerminal).for_each_interval([&](StateNrType state_nr_next_old_, Interval<SymbolValueType> interval_) {
   follow_and_copy_terminal_transitions(args_, locals_, interval_, state_nr_next_old_, state_new_);
 });
  
 state_old_.get_symbol_transitions(SymbolKindOpen).for_each_interval([&](StateNrType state_nr_next_old_, Interval<SymbolValueType> interval_) {
   follow_and_copy_open_transitions(args_, locals_, interval_, state_nr_next_old_, state_new_);
 });
  
 state_old_.get_symbol_transitions(SymbolKindConflict).for_each_interval([&](StateNrType state_nr_next_old_, Interval<SymbolValueType> interval_) {
  follow_and_copy_conflict_transitions(args_, locals_, interval_, state_nr_next_old_, state_new_);
 });
}
...
auto TerminalOverlapChecker::find_overlaps(Args args_) -> std::unordered_set<IntervalSet<AcceptsIndexType>> {
 ...
 args_._state_machine.get_state_nrs().for_each_key([&](StateNrType state_nr_) {
  ...
  close_transitions.for_each_key([&](StateNrType const state_nr_next_old_, SymbolValueType symbol_) {
   locals._post_close_state_nrs_old[symbol_].insert(state_nr_next_old_);
  });
 });

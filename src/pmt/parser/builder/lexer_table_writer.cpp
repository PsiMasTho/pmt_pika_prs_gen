...
void LexerTableWriter::replace_accepts(std::string& str_) {
  ...
  _writer_args->_tables.get_lexer_state_machine().get_state_nrs().for_each_key([&](StateNrType state_nr_) {
...
void LexerTableWriter::replace_linecount_accepts(std::string& str_) {
  ...
  _writer_args->_tables.get_linecount_state_machine().get_state_nrs().for_each_key([&](StateNrType state_nr_) {

...
auto main(int argc, char const* const* argv) -> int try {
 ...
 if (args._input_test_file.has_value()) {
  ...
  if (ast_testfile != nullptr) {
   GenericAstPrinter::print(GenericAstPrinter::Args{._id_to_string_fn = [&](GenericId::IdType id_) { return program.get_id_table().id_to_string(id_); }, ._out = std::cout, ._ast = *ast_testfile, ._indent_width = 2});
  }
 }

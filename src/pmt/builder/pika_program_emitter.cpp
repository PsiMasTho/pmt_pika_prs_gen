...
void PikaProgramEmitter::emit() {
 ...
 std::span<StateNrType const> const state_nrs = state_machine.get_state_nrs();
 ...
 for (size_t state_nr = 0; state_nr < state_count; ++state_nr) {
  ...
  if (state != nullptr) {
   state->get_symbol_transitions().for_each_interval([&](StateNrType const& next_state_, auto interval_) {
 ...
 std::vector<uint64_t> terminal_final_ids;
 std::vector<uint64_t> terminal_final_ids_offsets;
 terminal_final_ids_offsets.push_back(0);
 for (size_t state_nr = 0; state_nr < state_count; ++state_nr) {
  size_t const accept_count = terminal_tables.get_state_final_id_count(state_nr);
  terminal_final_ids_offsets.push_back(accept_count + terminal_final_ids_offsets.back());
  for (size_t i = 0; i < accept_count; ++i) {
   terminal_final_ids.push_back(terminal_tables.get_state_final_id(state_nr, i));
  }
 }
 ...
 for (Bitset::ChunkType chunk : clause_can_match_zero.get_chunks()) {
 ...
 size_t const rule_chunk_count = Bitset::get_required_chunk_count(rule_count);
 std::vector<uint64_t> rule_parameter_booleans(rule_chunk_count * 3, 0);
 if (rule_chunk_count != 0) {
  size_t const rule_bits_per_set = rule_chunk_count * Bitset::ChunkBit;
  for (size_t rule_id = 0; rule_id < rule_count; ++rule_id) {
 ...
 auto const terminal_final_ids_type = pick_unsigned_type(max_value(terminal_final_ids));
 auto const terminal_final_ids_offsets_type = pick_unsigned_type(max_value(terminal_final_ids_offsets));
 ...
 replace_skeleton_label(source, "TERMINAL_ACCEPTS_SIZE", format_hex(terminal_final_ids.size(), hex_digits(terminal_final_ids.size())));

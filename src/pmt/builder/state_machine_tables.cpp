...
auto StateMachineTables::get_state_final_id(StateNrType state_nr_, size_t index_) const -> FinalIdType {
 assert(state_nr_ < _state_final_ids_cache.size());
...
void StateMachineTables::init_cache() {
 ...
 for (StateNrType state_nr = 0; state_nr < _state_machine.get_state_count(); ++state_nr) {
  ...
  if (state) {
   state->get_final_ids().for_each_interval([&](Interval<FinalIdType> interval_) {
    for (FinalIdType accept = interval_.get_lower(); accept <= interval_.get_upper(); ++accept) {

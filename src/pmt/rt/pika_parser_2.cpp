...
auto PikaParser::memo_table_to_ast(MemoTable const& memo_table_, std::string_view input_, PikaProgramBase const& pika_program_) -> Ast::UniqueHandle {
 ...
 if (MemoTable::IndexType const start_match_index = memo_table_.find(MemoTable::Key{._clause = &pika_program_.fetch_clause(0), ._position = 0}, input_, pika_program_); start_match_index != MemoTable::MemoIndexMatchZeroLength) {

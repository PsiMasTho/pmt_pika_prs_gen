...
auto match_sequence(MemoTable const& memo_table_, MemoTable::Key key_, std::string_view input_, PikaProgramBase const& pika_program_) -> std::optional<MemoTable::Match> {
 ...
 for (size_t i = 0; i < key_._clause->get_child_id_count(); ++i) {
  ...
  MemoTable::Key child_key{._clause = &pika_program_.fetch_clause(child_id), ._position = key_._position + length};
  MemoTable::IndexType const child_match_index = memo_table_.find(child_key, input_, pika_program_);
...
auto match_choice(MemoTable const& memo_table_, MemoTable::Key key_, std::string_view input_, PikaProgramBase const& pika_program_) -> std::optional<MemoTable::Match> {
 for (size_t i = 0; i < key_._clause->get_child_id_count(); ++i) {
  ClauseBase::IdType const child_id = key_._clause->get_child_id_at(i);
  MemoTable::Key child_key{._clause = &pika_program_.fetch_clause(child_id), ._position = key_._position};
  MemoTable::IndexType const child_match_index = memo_table_.find(child_key, input_, pika_program_);
  if (child_match_index != MemoTable::MemoIndexMatchNotFound) {
...
auto match_one_or_more(MemoTable const& memo_table_, MemoTable::Key key_, std::string_view input_, PikaProgramBase const& pika_program_) -> std::optional<MemoTable::Match> {
 ...
 MemoTable::IndexType const tail_match_index = memo_table_.find(tail_key, input_, pika_program_);
...
auto ClauseMatcher::match(MemoTable const& memo_table_, MemoTable::Key key_, std::string_view input_, PikaProgramBase const& pika_program_) -> std::optional<MemoTable::Match> {
 switch (key_._clause->get_tag()) {
  ...
  case ClauseBase::Tag::Choice:
   return match_choice(memo_table_, key_, input_, pika_program_);
  case ClauseBase::Tag::OneOrMore:
   return match_one_or_more(memo_table_, key_, input_, pika_program_);
  case ClauseBase::Tag::NegativeLookahead:
   return match_not_followed_by(memo_table_, key_, input_, pika_program_);
  case ClauseBase::Tag::Epsilon:
   return match_epsilon(memo_table_, key_, input_, pika_program_);
  case ClauseBase::Tag::Identifier:
   return match_identifier(memo_table_, key_, input_, pika_program_);

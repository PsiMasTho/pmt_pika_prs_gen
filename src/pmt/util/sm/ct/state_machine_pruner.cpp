...
void copy_symbol_transitions(Locals& locals_, StateMachine& state_machine_new_, State const& state_old_, State& state_new_) {
 state_old_.get_symbol_kinds().for_each_key([&](SymbolKindType kind_) { state_old_.get_symbol_transitions(kind_).for_each_interval([&](StateNrType state_nr_next_old_, Interval<SymbolValueType> const& interval_) { state_new_.add_symbol_transition(kind_, interval_, locals_._old_to_new.find(state_nr_next_old_)->second); }); });
}

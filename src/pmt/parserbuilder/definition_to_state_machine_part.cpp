...
auto DefinitionToStateMachinePart::convert(pmt::util::smct::StateMachine& dest_state_machine_, std::string const& name_, pmt::util::smrt::GenericAstPath const& definition_, std::vector<std::string> const& names_, std::vector<pmt::util::smrt::GenericAstPath> const& definitions_, pmt::util::smrt::GenericAst const& ast_root_) -> pmt::util::smct::StateMachinePart {
  ExpressionToStateMachinePartFrameBase::CallstackType callstack;
  ExpressionToStateMachinePartFrameBaseCaptures captures{._dest_state_machine = dest_state_machine_, ._names = names_, ._definitions = definitions_, ._ast = ast_root_};

  captures._name_stack_contents.insert(name_);
  captures._name_stack.push_back(name_);
  callstack.push(ExpressionToStateMachinePartFrameFactory::construct(ast_root_, definition_));

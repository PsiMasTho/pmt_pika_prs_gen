...
void StateMachineToTransitionTables<TAG_>::step_2(Context& context_) {
  std::unordered_set<pmt::util::parsect::State::StateNrType> unvisited;
  ...
  while (!unvisited.empty()) {
    ...
    pmt::util::parsect::State::StateNrType state_nr_best = std::numeric_limits<pmt::util::parsect::State::StateNrType>::max();

    for (pmt::util::parsect::State::StateNrType i : unvisited) {
      double avg_diff = 0.f;
      for (pmt::util::parsect::State::StateNrType j : unvisited) {
        if (i == j) {
          continue;
        }
  ...
  if (std::optional<pmt::util::parsect::State::StateNrType> const state_nr_sink = context_._fa_with_sink.get_state_nr_sink()) {
...
void StateMachineToTransitionTables<TAG_>::step_3(Context& context_) {
  ...
  for (size_t i = 0; i < context_._ordering.size(); ++i) {
    pmt::util::parsect::State::StateNrType const state_nr_i = context_._ordering[i];

    pmt::util::parsect::State::StateNrType state_nr_best = context_._ordering.front();
    size_t popcnt_best = context_._diff_mat_2d[index_2d_to_1d(state_nr_i, state_nr_best, context_._fa_with_sink.get_size())].popcnt();

    for (pmt::util::parsect::State::StateNrType j = 0; j < i; ++j) {
      pmt::util::parsect::State::StateNrType const state_nr_j = context_._ordering[j];
...
void StateMachineToTransitionTables<TAG_>::step_4(Context& context_) {
  std::unordered_map<pmt::util::parsect::State::StateNrType, uint64_t> shifts;
...
auto StateMachineToTransitionTables<TAG_>::debug_post_checks(Context const& context_) -> bool {
  ...
  for (size_t i = 0; i < context_._fa_with_sink.get_size(); ++i) {
    for (size_t j = context_._alphabet_limits.first; j <= context_._alphabet_limits.second; ++j) {
      pmt::util::parsect::State::StateNrType const state_nr_next_fa = context_._fa_with_sink.get_state_nr_next(i, j);
      pmt::util::parsect::State::StateNrType const state_nr_next = context_._tables.get_state_nr_next(i, j);

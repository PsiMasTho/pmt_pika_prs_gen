...
auto Ast::construct(Tag tag_, IdType id_) -> UniqueHandle {
...
auto Ast::get_id() const -> IdType {
 return _id;
}

void Ast::set_id(IdType id_) {
 _id = id_;
}
...
auto Ast::get_tag() const -> Tag {
 ...
 assert(std::holds_alternative<ParentType>(_data));
 return Tag::Parent;
}
...
void Ast::set_string(StringType string_) {
 assert(get_tag() == Tag::String);
 _data.emplace<StringType>(std::move(string_));
}

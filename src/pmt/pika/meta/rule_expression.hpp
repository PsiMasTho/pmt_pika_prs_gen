...
private:
 ...
 // -$ Functions $-
 // --$ Lifetime $--
 explicit RuleExpression(pmt::pika::rt::ClauseBase::Tag tag_);

public:
 static void destruct(RuleExpression* self_);
 static auto construct(pmt::pika::rt::ClauseBase::Tag tag_) -> UniqueHandle;

 static auto clone(RuleExpression const& other_) -> UniqueHandle;

 // --$ Other $--
 static void swap(RuleExpression& lhs_, RuleExpression& rhs_);

 auto get_tag() const -> pmt::pika::rt::ClauseBase::Tag;

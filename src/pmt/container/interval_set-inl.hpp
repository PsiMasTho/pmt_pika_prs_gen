...
void IntervalSet<KEY_>::insert(Interval<KEY_> interval_) {
 ...
 pmt::util::move_bidir(lowers.begin() + k, lowers.begin() + k + shift, lowers.begin() + indices.first._idx + 1);
 pmt::util::move_bidir(uppers.begin() + k, uppers.begin() + k + shift, uppers.begin() + indices.first._idx + 1);
...
namespace std {
template <std::integral KEY_>
auto hash<pmt::container::IntervalSet<KEY_>>::operator()(pmt::container::IntervalSet<KEY_> const& interval_set_) const -> size_t {
 return interval_set_.hash();
}
}  // namespace std
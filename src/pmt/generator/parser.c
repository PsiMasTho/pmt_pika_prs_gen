...
static size_t parser_remaining(parser* self_) {
  return self_->_end - self_->_cursor;
}

bool parse_eof(parser* self_) {
  return parser_remaining(self_) == 0;
}

bool parse_newline(parser* self_) {
  if (parse_eof(self_))
    return false;
...
bool parse_eolf(parser* self_) {
  return parse_eof(self_) || parse_newline(self_);
}

bool skip_ws(parser* self_) {
...
bool parse_identifier(parser* self_, ast_node* ret_) {
...
bool parse_dependency_list(parser* self_, ast_node* ret_) {
...
bool parse_dependency_stmnt(parser* self_, ast_node* ret_) {
  ast_node identifier = ast_node_create(GRAMMAR_INFO._entries[GRAMMAR_ID_IDENTIFIER]._tag, GRAMMAR_ID_IDENTIFIER);
  char* backtrack = self_->_cursor;
...
bool parse_identifier_stmnt(parser* self_, ast_node* ret_) {
  char* backtrack = self_->_cursor;
...
bool parse_dependency_body(parser* self_, ast_node* ret_) {
  ret_->_union._children = ast_node_children_create();
...
bool parse_identifier_body(parser* self_, ast_node* ret_) {
  ret_->_union._children = ast_node_children_create();
...
bool parse_section(parser* self_, ast_node* ret_) {
  char* backtrack = self_->_cursor;
...
bool parse_file(parser* self_, ast_node* ret_) {
  ret_->_union._children = ast_node_children_create();

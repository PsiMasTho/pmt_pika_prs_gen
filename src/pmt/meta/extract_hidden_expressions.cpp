...
auto count_unique_hidden_expressions(std::span<AstPosition const> hidden_expressions_) -> size_t {
 std::unordered_set<AstNodeKey, AstNodeHash, AstNodeEq> unique_set;
 for (AstPosition const& pos : hidden_expressions_) {
  unique_set.insert(AstNodeKey{pos._parent->get_child_at(pos._child_idx)});
 }
...
void extract_hidden_expressions(pmt::rt::Ast& ast_) {
 ...
 std::unordered_map<AstNodeKey, std::string, AstNodeHash, AstNodeEq> hidden_expression_to_rule_name;
 ...
 for (size_t i = 0; AstPosition const& pos : hidden_expressions) {
  AstNodeKey const pos_deep{hidden_expressions_orig[i]->get_child_at(0)};

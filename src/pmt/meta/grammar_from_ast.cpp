...
void initial_traversal_handle_grammar_property_start(Locals& locals_, Ast const& ast_) {
 assert(ast_.get_id() == Language::Identifier);
 locals_._ret.set_start_rule_name(ast_.get_string());
}

void initial_traversal_handle_grammar_property(Locals& locals_, Ast const& ast_) {
...
void initial_traversal_handle_production(Locals& locals_, Ast const& ast_) {
...
void initial_traversal(Locals& locals_, Ast const& ast_) {
...
auto GrammarFromAst::make(pmt::rt::Ast const& ast_) -> Grammar {
 Locals locals;
 initial_traversal(locals, ast_);

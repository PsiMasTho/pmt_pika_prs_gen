...
auto main(int argc, char** argv) -> int {
 ...
 pmt::pika::rt::MemoTable const memo = pmt::pika::rt::PikaParser::populate_memo_table(input, tables);

 // Construct the AST
 pmt::pika::rt::Ast::UniqueHandle const ast = pmt::pika::rt::PikaParser::memo_table_to_ast(memo, input, tables);
 ...
 std::vector<pmt::pika::rt::Ast const*> pending{ast.get()};
 while (!pending.empty()) {
  pmt::pika::rt::Ast const& cur = *pending.back();
  pending.pop_back();
  ...
  if (cur.get_tag() == pmt::pika::rt::Ast::Tag::Parent) {
 ...
 // Print the whole ast
 pmt::pika::rt::AstToString const ast_to_string(example::Ids::id_to_string, 2);
 std::cout << ast_to_string.to_string(*ast);
}

// Notes:
//  - The lifetime of pmt::pika::rt::Ast objects should only ever be managed through a UniqueHandle.
//    That means don't manually heap allocate any pmt::pika::rt::Ast or even put them on the stack. Instead, use the pmt::pika::rt::Ast::construct function to get a UniqueHandle
